...
def load_all_bboxes():
    gt_bboxes = {}
    with open('/home/inho/music/music_solo.json') as json_file:
...
class AudioVisualDataset(Dataset):
    ...
    def getitem_train(self, idx):
        ...
        audio_fn = os.path.join(self.audio_path, self.audio_files[idx])
        spectrogram, audio_ss = load_spectrogram_torchaudio(audio_fn, dur=self.audio_dur, rand=self.rand_aud)
        # spectrogram = torch.from_numpy(spectrogram)
        spectrogram = self.aud_transform(spectrogram)
...
def get_train_dataset(args, hard_img, hard_aud, rand_aud):
    audio_path = os.path.join(args.train_data_path, 'audios')
    image_path = os.path.join(args.train_data_path, 'frames')
    ...
    subset = set([line.split(',')[0] for line in open(f"{args.train_data_path}/{args.trainset}_train.txt").read().splitlines()])
...
def get_test_dataset(args):
    root_dir = '/home/inho/music/'
    
    testcsv = os.path.join(root_dir, "music_solo_test.csv")
    audio_path = os.path.join(root_dir, "audios/")
